...
We observe comparable fold-changes across both weakened and strengthened loops, compared to the changes observed in the paper (extended data figure 8).
However, the ratio of strengthened to weakened loops varies wildly between this analysis and the papers findings ad previously noted.
...
def load_pileup(path):
    ...
    for col in df.columns:
        ...
        if isinstance(val, np.ndarray) and val.ndim == 2:
            return np.nanmean(np.stack(df[col].values), axis=0)
    if df.shape == (1,):
        mat = df.iloc[0, 0]
        if isinstance(mat, np.ndarray):
            return mat
    raise ValueError("Could not extract 2‐D pile‐up matrix.")

def plot_apa_pair(path_mock, path_cov, title_mock, title_cov, bin_size_kb=40):
    ...
    for ax, mat, title in zip(axes, [mat_mock, mat_cov], [title_mock, title_cov]):
        ...
        half_bin = bin_size_kb / 2
        ax.add_patch(patches.Rectangle(
            (-half_bin, -half_bin), bin_size_kb, bin_size_kb,
            fill=False, edgecolor="cyan", linewidth=2.0
        ))

        centre_val = mat[centre, centre]
        ax.text(0.05, 0.92, f"P2LL: {centre_val:.2f}", transform=ax.transAxes,
