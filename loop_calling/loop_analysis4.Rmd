...
Structural chromatin loops are formed primarily by cohesin-driven loop extrusion and
anchored at convergent CTCF sites. The paper shows that SARS-CoV-2 infection
preferentially depletes cohesin from intra-TAD regions, weakening these loops and
...
| Aspect | Paper | This analysis |
|---|---|---|
| Chromosomes | Genome-wide | chr14 only |
| Resolutions | 5 kb + 10 kb | **10 kb only** — processed mcool files do not contain 5 kb bins |
| Loop merging | Full 4DN 5 kb + 10 kb strategy | Simplified to 10 kb union (single resolution) |
...
Find all loops that occurr in both conditions.
...
cat(sprintf("Loops called in Mock:        %d\n", nrow(loops_mock)))
cat(sprintf("Loops called in SARS-CoV-2:  %d\n", nrow(loops_cov)))
```
...
cat(sprintf("Loops reproducibly called in both conditions: %d\n", nrow(shared)))
cat(sprintf(
  "Loops unique to Mock:        %d\n",
  nrow(loops_mock) - length(unique(shared$queryHits))
))
cat(sprintf(
  "Loops unique to SARS-CoV-2:  %d\n",
  nrow(loops_cov) - length(unique(shared$subjectHits))
))
```
...
cat(sprintf(
  "Consensus loop set: %d unique loops on chr14\n",
  nrow(loops_consensus)
))
```
...
```{r}
ggplot(loops_consensus, aes(x = loop_size_kb)) +
  geom_histogram(
    binwidth = 50, fill = "#4D9221", colour = "white",
    linewidth = 0.2
  ) +
  scale_x_continuous(
    labels = comma_format(suffix = " kb"),
    limits = c(0, NA)
  ) +
  labs(
    x     = "Loop span (kb)",
    y     = "Number of loops",
    title = "Consensus loop set — size distribution"
  ) +
  theme_classic(base_size = 13)
```

Most loops are below 2000kb size.
...
print(f"Saved count matrix: {count_df.shape[0]} loops at 10kb resolution")
```
...
cat(sprintf(
  "Loops retained after count filtering: %d / %d\n",
